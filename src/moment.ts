...
function getSupportedTimeZones(): string[] {
  ...
  if (typeof maybeIntl.supportedValuesOf === 'function') {
    cachedTimeZones = maybeIntl.supportedValuesOf('timeZone');
  } else {
    cachedTimeZones = [moment.tz.guess()];
  }
...
function parseWithFormats(
  ...
  for (const fmt of formats) {
    const parsed = fmt === ISO_8601 ? DateTime.fromISO(value, options) : DateTime.fromFormat(value, fmt, options);
    if (parsed.isValid) {
      return parsed;
    }
  }
...
function makeMoment(
  ...
  const api: MomentLike = {
    ...
    format(template = DEFAULT_MOMENT_FORMAT) {
